 ...
 */
export function setInputState(input, button, nextState) {
    switch (nextState) {
        case "pending":
            input.disabled = true;
 ...
 */
export async function runSendLifecycle(opts) {
    ...
    const [success] = await Promise.all([
        doSend(),
        new Promise((r) => setTimeout(r, MIN_LOCK_DURATION_MS)),
    ]);
 ...
 */
export function autoResizeTextarea(input, maxHeight = 120) {
    input.style.height = "auto";
