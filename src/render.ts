import type { ChatMessage, DOMRefs, RenderOptions } from "./types.js";
...
export function renderMessageText(raw: string): string {
  const parts: string[] = [];
...
function renderInlineCode(escaped: string): string {
...
export function renderMessageBubble(
  ...
  const isOutgoing: boolean = msg.sender_name === state.localUser;
  ...
  const replyPreviewHtml = (msg as ChatMessage & { replyTo?: { sender_name: string; message_text: string } }).replyTo
    ? `<div class="reply-preview">
         <span class="reply-sender">${escapeHtml((msg as any).replyTo.sender_name)}</span>
         <span class="reply-text">${escapeHtml((msg as any).replyTo.message_text.slice(0, 80))}</span>
       </div>`
    : "";
  ...
  node.querySelectorAll<HTMLButtonElement>(".copy-code-btn").forEach((btn) => {
    btn.addEventListener("click", (e) => {
      e.stopPropagation();
      const pre = btn.closest(".code-block-header")?.nextElementSibling as HTMLPreElement | null;
      if (pre) {
...
export function renderAllMessages(refs: DOMRefs): void {
