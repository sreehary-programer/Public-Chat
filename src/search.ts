...
if (!(searchResultsEl instanceof HTMLDivElement)) {
  ...
  const closeSearchBtn =
    document.getElementById("closeSearch") as HTMLButtonElement | null;

  const emojiPicker =
    document.getElementById("emojiPicker") as HTMLDivElement | null;
  ...
  document
    .querySelectorAll<HTMLElement>(".search-trigger")
    .forEach((btn) => {
  ...
  searchInput.addEventListener("input", () => {
    ...
    const matches = state.activeMessages.filter((m) =>
    ...
    matches.forEach((msg) => {
      ...
      item.addEventListener("click", () => {
        ...
        const el =
          refs.messageStream.querySelector<HTMLElement>(
