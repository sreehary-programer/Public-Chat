 ...
 */
export function initConnectionBanner(hostEl: HTMLElement): void {
  if (bannerEl) return; // idempotent
...
const STATUS_CONFIG: Record<
  ConnectionStatus,
  { modifier: string; dot: string; label: string }
> = {
...
function updateBanner(next: ConnectionStatus): void {
  if (!bannerEl || next === currentStatus) return;
  currentStatus = next;
  ...
  (bannerEl.querySelector(".conn-dot") as HTMLElement).textContent  = cfg.dot;
  (bannerEl.querySelector(".conn-label") as HTMLElement).textContent = cfg.label;
}

// ─── Signal sources ───────────────────────────────────────────────────────────

/** Derives the current status purely from the browser's navigator.onLine. */
function deriveFromNavigator(): ConnectionStatus {
  return navigator.onLine ? "connected" : "offline";
}

/** Called by main.ts when the Supabase channel reports a status change. */
export function updateFromRealtimeStatus(realtimeStatus: string): void {
  if (!navigator.onLine) {
  ...
  switch (realtimeStatus) {
    ...
    case "SUBSCRIBING":
      ...
      if (currentStatus !== "connected") updateBanner("reconnecting");
      break;
    default:
      break;
  }
