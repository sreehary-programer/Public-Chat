...
// ─── 1. Historical load ───────────────────────────────────────────────────────
export async function loadHistoricalMessages(refs) {
    ...
    if (data) {
        state.activeMessages = data.map(rowToMessage);
        renderAllMessages(refs);
    }
}
// ─── 2. Load reactions for the active channel ─────────────────────────────────
export async function loadReactions(refs) {
    ...
    for (const row of data) {
        ...
        if (!state.reactions[message_id])
            state.reactions[message_id] = {};
        state.reactions[message_id][emoji] = (state.reactions[message_id][emoji] || 0) + 1;
        if (sender_name === state.localUser) {
            if (!state.userReactions[message_id])
                state.userReactions[message_id] = new Set();
...
// ─── 3. Realtime listener ─────────────────────────────────────────────────────
export function initializeRealtimeListener(refs) {
    ...
    const channel = supabase
        ...
        const alreadyPresent = state.activeMessages.some((m) => m.id === row.id);
        if (alreadyPresent)
            return;
        // Fetch reply snapshot if this message has one
        let replied_msg = null;
        if (row.reply_to_id) {
        ...
        const incoming = rowToMessage({ ...row, replied_msg });
...
export async function sendMessage(text, refs, replyTo) {
    ...
    if (data) {
        ...
        if (actionsEl)
            actionsEl.dataset.msgId = data.id;
    }
    return true;
}
// ─── 5. Toggle reaction in Supabase ──────────────────────────────────────────
export async function persistReactionToggle(msgId, emoji, isAdding) {
    if (isAdding) {
...
// ─── UI helpers ───────────────────────────────────────────────────────────────
function setSendButtonState(btn, sendState) {
